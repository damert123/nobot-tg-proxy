...
//                    Log::channel('tg-messages')->info("Информация о медиа:" . json_encode($media, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
    ...
    public function onUpdateEditMessage(array $update): void
    {
        ...
        if ($message) {
            ...
            Log::channel('tg-messages')->info("Получено редактируемое сообщение", [
                'peer_id' => $peerId,
                'from_id' => $fromId,
                'manager_id' => $managerId,
                'message' => $text,
            ...
            if ($responseGetTask->successful() && !empty($responseGetTask->json())) {
                ...
                $data = [
                    ...
                    'chatId' => $clientId,
                    'planfix_token' => $planfixIntegration->planfix_token,
                    'message' => self::EDIT_PREFIX . $text ?: 'Измененный файл',
                    'contactId' => $fromId,
                    'contactName' => $senderFirstName,
                    'contactLastName' => $senderLastName,
            ...
            // Логируем финальное сообщение
            Log::channel('tg-messages')->info("Редактируемое сообщение: {$text}, от пользователя: {$fromId}, username: {$clientUserName}, имя: {$senderFirstName}, фамилия: {$senderLastName}");
